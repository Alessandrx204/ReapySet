...
import src.ReadySet.lang_specific_buttons
from src.ReadySet.mainWindow import init_main_window


class LangSetup:

    def __init__(self, p_window, p_languages, p_per_row=5):
        self.language_buttons = init_main_window(p_window, p_languages, p_per_row)
    @staticmethod
    def setup_buttons(p_window, p_chosen_lang, p_buttons: list[QPushButton]):
        match p_chosen_lang:
            case "Python":
                src.ReadySet.lang_specific_buttons.LangSpecificButtons.init_python(p_window, p_buttons)
    ...
    @staticmethod
    def common_window_update(p_window, p_buttons: list[QPushButton]):
        ...
        if not hasattr(p_window, "back_button"): #back button
            p_window.back_button = QPushButton("Back", p_window)
            p_window.back_button.setFixedSize(90, 32)
            p_window.back_button.hide()

            p_window.back_button.clicked.connect(lambda: (
                LangSetup.restore_window(p_window),
                LangSetup.enable_language_buttons(p_buttons)
            ))#restores og window size
    ...
    @staticmethod
    def disable_language_buttons(p_buttons: list[QPushButton]):
        for btn in p_buttons:
            btn.setEnabled(False)
    @staticmethod
    def enable_language_buttons(p_buttons: list[QPushButton]):
        for btn in p_buttons:
            btn.setEnabled(True)
...
def error_window(p_parent=None, p_title="Error", p_text="Something Went Wrong", p_url="https://www.python.org"):
    msg = QMessageBox(p_parent)
    msg.setWindowTitle(p_title)
    msg.setText(p_text)
    msg.setIcon(QMessageBox.Icon.Critical)

    learn_more: QPushButton = msg.addButton("Learn More", QMessageBox.ButtonRole.HelpRole)
